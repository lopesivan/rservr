...
#define ALL_RQSRVC_SOURCE_INFO_GETATTR(macro) \
macro(rqsrvc_source_info, origin) \
macro(rqsrvc_source_info, target) \
macro(rqsrvc_source_info, sender) \
macro(rqsrvc_source_info, address)


TYPE_GETATTR(rqsrvc_source_info, origin,  Py_BuildValue("s", SELF->pointer->origin))
TYPE_GETATTR(rqsrvc_source_info, target,  Py_BuildValue("s", SELF->pointer->target))
TYPE_GETATTR(rqsrvc_source_info, sender,  Py_BuildValue("s", SELF->pointer->sender))
TYPE_GETATTR(rqsrvc_source_info, address, Py_BuildValue("s", SELF->pointer->address))
...
PyMODINIT_FUNC initrsvp_rqsrvc(void)
{
	...
	ALL_RQSRVC_SOURCE_INFO_GETATTR(TYPE_GETATTR_REGISTER)
	if (!MODULE) return;
}
